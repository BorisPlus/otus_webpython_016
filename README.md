...
Пусть у нас на страницу выводятся 25 товаров. Я сделал так, чтоб и малого импорта данных мне было достаточно для возникновения пагинации. Хотя в настоящее время, проблем с массовым импортом реальных "магазинных" данных проект не испытывает.
 
 Так как я использую авторизацию в админке, то на всё про всё приходится 80 запросов (зарпосы к _django_session_ и _auth_user_ включены).
...
И в итогк запрос свзанных внешних ключей таков
...
Стоит изменить число выбираемых товаров на странице, мы увидем что число запросов растет абсоюлно соразмерно-пропорционально. Увеличим число товаров на странице на 5, число запросов тоже подскочит на 5, а если на 25 - то и на 25.
...
Запростов стало... **6**!!! шесть, жесть, шесть...

Увеличим число товаров на странице до 100. Запросов по прежднему всего **6**!!!.

На моих данных (из-за числа попавших в выборку атрибутов и городов), если просто закоменнетировать `get_queryset`, выборка 100 товаров на страницу приведет к **499** запросам! Что **в 83 раза больше 6**! Пока отложим дальнейшие манипуляции с этой моделью в админке.
