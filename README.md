...
* select_related;
* prefetch_related;
* annotation, agregation, Subquery (заняло один пункт в списке, так как в этом примере они объеденены в одну подзадачу, но это вовсе не означает, что обособленное друг от друга их использование является нецелесообразным).
...
Спискок сущностей немного надуманный, он подобран для демонстрации возможности оптимизации запросов Django ORM к базе данных.
...
* название товара;
* описание товара;
* основное изображение товара (про работу с картинками я описал ранее в [ДЗ 6](https://github.com/BorisPlus/otus_webpython_006), но я повторюсь в конце данного отчета. Планирую реализовать отдельный пакет);
* список атрибутов товара.

Пусть у нас на страницу выводится список из 25 товаров. Я так сделал, чтоб и малого импорта данных мне было достаточно для возникновения пагинации. Хотя в настоящее время, проблем с массовым импортом реальных "магазинных" данных проект не испытывает (см. [пункт](https://github.com/BorisPlus/otus_webpython_016#Загрузка-БД-тестовыми-свдениями)). Реализован онлайн сбор данных из сайта Интернет-магазина "Мвидео".
 ...
 Итак. Товаров **25**, а запросов **80**. Понятно, что идет отбор соответствующих внешних ключей (магазин, город магазина) и свяей многие-ко-многим (список атрибутов и их значений). **Но 80 - это ж более чем в 3 раза больше, чем 25!!!**
...
Теперь вспомните пункт ["запомните этот пункт"](https://github.com/BorisPlus/otus_webpython_016#attention_001), в товарах есть обращение к городу магазина, где продается этот товар, а это внешний ключ (город) внешнего ключа (магазин) сущности (товар).
...
В итоге запрос связанных внешних ключей таков:
...
если использовать только `qs = qs.select_related('shop__city')`, то этого вполне будет достаточно для оптимизации выборки сущностей внешних ключей со всеми промежуточными, так как чтобы добраться по краних таких сущностей (внешний ключ внешнего ключа), запросу все равно приходится выбирать промежуточные внешние ключи, чьи внешние ключи потом он в итоге и выберет. Просто так устроен реляционный язык запросов (будь то SQL или еще какой *QL). Чтоб добраться до объекта-связи текущего объекта-родителя, нужны все промежуточные связи. Как то так...
...
```python
 ...
 ...
```
и число запросов останется преждним - 30 запросов, да и SQL запрос то же тот же.

Продолжим.

Стоит изменить число выбираемых товаров на странице, мы увидим, что число запросов растет абсолютно соразмерно-пропорционально. Увеличим число товаров на странице на 5, число запросов то же возрастет на 5, а если на 25 - то и запросов на 25.
...
Как ни странно - число запросов возрасло до 31, то есть стало на 1 запрос больше. А все потому, что при таком подходе (при обращении через метод _values_list_) ORM не знает об оптимизированой выборке у моделей. Для оптимизированного ORM необходимо использование list-comprehension подхода к выборке полей связанных M2M-сущностей.
...
На моих данных (из-за числа попавших в выборку атрибутов и городов), если просто закоменнетировать `get_queryset`, выборка 100 товаров на страницу приведет к **499** запросам! Что **в 83 раза больше 6**! Пока отложим дальнейшие манипуляции с этой моделью в админке. Если говорить о времени, то это были 93 миллисекунды против 3 милисекунд после оптимизации.
...
Это выражение сложно воспринимается, но это абсолютно то же, что вы бы сделали "ручками", если б писали чистый SQL запрос. Итак.
 ...
 Мы, _во-первых_, агрегировали число (см. "Count('id')") товаров, имеющихся в магазинах (shop), применив _OuterRef_, для отсылки выбираемых товаров на магазин.
  
  Поскольку это число, то обернем значение этого типа в _IntegerField()_.
 
 А _во-вторых_, аннотировали агрегированное значение.
 
 Это делается не для конкретного объекта, а для всех в выборке, что через Subquery и аннотирование дает результирующий целочисленный столбец с числом товаров в магазине. Обращение к этому столбцу реализовано в отдельном custom-методе администратора модели:
...
Аха. А что у нас тут с числом запросов? Оно так же неизменно. Тут **6** SQL-запрососов.
...
Так, число запросов осталось неизменным... равным **14**, а должно быть **6 + 1**.

Но мы забыли. У магазина есть внешний ключ на город, который мы используем в отображении полного наименования магазина в формате "Магазин (г.Город)". Вам не напомнило это задачу выборки внешнего ключа, связанного по внешнему ключу с объеком. Да! тут нужен уже известный метод _select_related_.
...
В модели Product имеется три поля, чье отображение в административной панели в виде текста является не эффективным, или точнее не "ориентированным" на пользователя:
* _url_at_shop_ - cсылка на товар в магазине
* _main_image_url_ - главное изображение
* _attributes_ - список атрибутов
    
Типовым подходом к реализации корректного отображения подобных полей в админке в виде HTML ссылки (<a ...>...</a>), изображения (<img ...>) или списка с разделителем (', ') принято считать создание нового метода у модели в админке с оборачиванием в HTML выводимого значения. Например:
...
Данный подход при наличии большого числа таких одинаковых по формату вывода полей (если честно, то при наличии **>= 2** полей уже дублирование кода есть), которые нужно обернуть в HTML, будет вынуждать реализовавать "пухлые" методы для каждого такого поля. Например, если у вас много моделей c полями для картинок или ссылок. И при возникновении необходимости внесения изменений в генерируемый HTML, например, при необходимости внесения _CSS_-атрибута _class_, понадобится внести эти изменения **везде** во все эти и без того "пухлые" методы. Я предлагаю усовершенствованый подход. 
...
Это декорирование приведет к тому, что ссылка на товар в магазине будет ввиде картинки. Ссылка откроется в новой вкладке. Сама ссылки берется из поля _url_at_shop_ объекта.
